...
export function BatchOnboardForm() {
  ...
  useEffect(() => {
    ...
    toast.success(
      `Successfully registered ${rows.length} policies on Arbitrum Sepolia.`,
    );
  }, [isSuccess, hash, rows.length]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    for (const row of rows) {
      ...
      const amount = Number(row.coverageUsdc);
      if (!Number.isFinite(amount) || amount <= 0) {
        toast.error("Coverage must be a positive USDC amount.");
        return;
      }
    }
    ...
    if (isDemoMode) {
      ...
      toast.success(
        `Demo: Successfully registered ${rows.length} policies!`,
      );
      return;
    }
    ...
    const coverageAmounts = rows.map((r) =>
      usdcToBaseUnits(Number(r.coverageUsdc)),
    );

    reset();
    notifiedHash.current = null;

    // Re-estimate right before send — Arbitrum Sepolia base fee often ticks
    // above a stale wallet maxFeePerGas between prompt and broadcast.
    ...
    >
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--accent)]">
            Add cooperative members, then register all policies in one Stylus
            transaction.
          </p>
        </div>
        <span
          ...
          >
            ...
            <label className="block space-y-1.5">
              ...
              <input
                ...
                step="1"
                inputMode="decimal"
                value={row.coverageUsdc}
                onChange={(ev) =>
                  updateRow(row.id, { coverageUsdc: ev.target.value })
                }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          disabled={busy}
          className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-sky-600 px-5 text-sm font-extrabold text-white shadow-[0_0_24px_rgba(2,132,199,0.35)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {busy ? (
